# Sky Siege Industrial

## コンセプト

**Sky Siege Industrial** は、地上が危険地帯となった世界で、空中に工業拠点・飛行船・移動要塞を築いて生き延びることを目的とした Minecraft 1.20.1 Forge 向け Modpack コンセプトです。

地上にはホード、強化 Mob、廃墟都市、ダンジョン、危険なバイオームが広がっています。プレイヤーは地上に恒久拠点を作らず、資源・戦利品・設計素材を求めて危険な地上へ降下し、回収後は空中拠点へ帰還します。

空は比較的安全ですが、資源はありません。地上は危険ですが、工業化・武装化・拠点拡張に必要なものが眠っています。

> 地上は危険。  
> 空は安全。  
> でも、冒険と資源は地上にしかない。

最終的な目標は、Create / Valkyrien Skies / Mekanism / AE2 / Immersive Engineering を使って、武装した空中工業要塞 **Sky Fortress** を完成させ、地上の危険地帯を攻略・奪還することです。

---

## 推奨環境

- Minecraft: **1.20.1**
- Mod Loader: **Forge**
- プレイ方針: 工業 + 空中拠点 + 物理演算 + 銃火器 + 高難易度サバイバル + 探索

---

## 現行 Mod 一覧

この一覧は `mods/*.pw.toml`、`tacz/*.pw.toml`、`shaderpacks/*.pw.toml` の現行メタデータに合わせています。

### コア・物理演算・空中拠点系

- Valkyrien Skies
- Eureka! Ships! for Valkyrien Skies (Forge/Fabric)
- Clockwork
- Trackwork
- Create
- Create: Interactive
- Create: Propulsion
- Create: Copycats+
- Create Renewability
- Valkyrien Warium
- Valkyrien Pirates
- Ship In A Bottle
- Carry On + Valkyrien Skies Compat

### 工業・ストレージ・自動化系

- Mekanism
- Mekanism Additions
- Mekanism Generators
- Mekanism Tools
- More Mekanism Processing
- Applied Energistics 2
- AdvancedAE
- AE2 MEGA Things
- AE2 Things [Forge]
- AEInfinityBooster
- Applied Mekanistics
- ExtendedAE
- ME Requester
- MEGA Cells
- Modern AE2 Additions
- Polymorphic Energistics
- Immersive Engineering
- More Immersive Wires
- Create Crafts & Additions
- Flux Networks
- Glodium
- PackagedAuto
- PackagedTacz - AE2 auto craft Tacz ammos
- Storage Drawers
- Iron Chests

### 武器・戦闘・TaCZ 系

- [TaCZ] Timeless and Classics Zero Guns
- Create: Timeless and Classics Zero [TaCZ]
- Create: TaCZ Automation
- Project TaCZ (Timeless & Classic Zero Addon)
- TaCZ addon
- TACZ Turrets
- Just Enough Guns
- TACZ Bullet Proof Enchant Forge Add-on
- TaCZ x Guns Lights Addon [NEW] - Update 2.4.3
- [TACZ] Gucci & Vuitton Attachments
- [TACZ] LesRaisins Tactical Equipements
- [SBW] Superb Warfare
- Fel's Machine Guns II
- FMGII: German Pack
- Pillager's Gun
- ChaosZProject: Bandits! - Pillager's Gun Addon
- Third Person Shooting: Zero
- Item Scrapper
- Item Scrapper Pack - TACZ

### TaCZ データパック

- [TaCZ] ARIPS Content Expansion Pack
- [TaCZ]Applied Armorer
- [TaCZ]Call of Duty Warzone Gunpack
- [TACZ]classics reborn
- [TaCZ]Create Armorer
- [TaCZ]Cyber Armorer
- [TacZ]Gunpowder Revolution:Great War gunpack
- [TaCZ]Immersive Armorer
- [TaCZ]Suffuse GunSmoke
- [TACZ]The Division gunpack
- 【TACZ】Anomaly Interception EX Pack

### 難易度・敵・進行系

- The Hordes
- Born in Chaos
- Enhanced AI
- Zombie Awareness
- Majrusz's Progressive Difficulty
- In Control!
- Apotheosis
- Apotheosis Modern Ragnarok: Zero
- Apothic Attributes
- MmmMmmMmmMmm (Target Dummy)

### バイオーム・地形・構造物系

- Regions Unexplored
- TerraBlender (Forge)
- Terralith
- Tectonic
- The Lost Castle [NeoForge/Fabric]
- Alex's Caves
- Dungeons Enhanced
- When Dungeons Arise - Forge!
- Towns and Towers
- ChoiceTheorem's Overhauled Village
- Repurposed Structures (Neoforge/Forge)
- MSS - Moog's Soaring Structures
- Moog's Structure Lib (moogs_structures)
- Structure Gel API
- Lithostitched
- Cristel Lib
- YUNG's API (Forge/NeoForge)
- YUNG's Better Desert Temples (Forge/NeoForge)
- YUNG's Better Dungeons (Forge/NeoForge)
- YUNG's Better End Island (Forge/NeoForge)
- YUNG's Better Jungle Temples (Forge/NeoForge)
- YUNG's Better Mineshafts (Forge/NeoForge)
- YUNG's Better Nether Fortresses (Forge/NeoForge)
- YUNG's Better Ocean Monuments (Forge/NeoForge)
- YUNG's Better Witch Huts (Forge/NeoForge)

### 食料・農業・生活系

- Farmer's Delight
- Chef's Delight [Forge] - Farmer's Delight Villagers
- End's Delight
- Festive Delight - Christmas eve
- Maturi Delight
- Nether's Delight
- Pam's HarvestCraft 2 - Food Extended
- Cooking for Blockheads
- [Let's Do] Vinery
- Animal Feeding Trough [Fabric | Forge | Quilt | NeoForge]
- Easy Villagers

### 探索・プレイヤー操作・ユーティリティ系

- Distant Horizons: A Level of Detail mod
- JourneyMap
- Nature's Compass
- Paragliders
- Hookshot - From Zelda
- Do a Barrel Roll
- Shoulder Surfing Reloaded
- Auto Third Person
- Cosmetic Armor Reworked
- Curious Lanterns
- Radiant Gear (Fabric/Forge/Quilt)
- GraveStone Mod
- PlayerRevive
- Carry On
- Construction Wand
- Hole Filler Mod
- IkkatsuHakai
- IkkatsuHakai Shovel
- KleeSlabs
- FallingTree
- Climb Ladders Fast [Fabric/Forge]
- Skin Layers 3D
- Yes Steve Model

### レシピ・情報表示・UI・QoL 系

- Just Enough Items (JEI)
- Just Enough Effect Descriptions (JEED)
- Just Enough Mekanism Multiblocks
- Just Enough Professions (JEP)
- Just Enough Resources (JER)
- Jade 🔍
- Jade Addons (Neo/Forge)
- JadeColonies
- The One Probe
- AppleSkin
- Better Advancements
- Configured
- Controlling
- Enchantment Descriptions
- FTB Backups 2
- FTB Library (NeoForge)
- FTB Quests (NeoForge)
- FTB Teams (NeoForge)
- Game Menu Mod Option [Forge]
- GuideME
- In-Game Account Switcher
- Inventory Profiles Next
- libIPN
- Loot Beams: Relooted!
- Lootr (Forge & NeoForge)
- Neat
- Ping Wheel
- Polymorph (Fabric/Forge/Quilt)
- RebindNarrator [Forge]
- Searchables
- Sophisticated Backpacks
- Sophisticated Core
- Tooltip Overhaul
- Untranslated Items
- What Are They Up To (Watut)

### 表示・音響・軽量化・安定化系

- Embeddium
- Embeddium (Rubidium) Extra
- Oculus
- Complementary Shaders - Unbound
- Accelerated Rendering
- Better Fps - Render Distance[Forge]
- Clumps
- Connectivity
- Entity Culling Fabric/Forge
- FerriteCore ((Neo)Forge)
- FastFurnace
- FastWorkbench
- Memory Leak Fix
- ModernFix
- Noisium
- Observable
- Packet Fixer
- XXL Packets
- AmbientSounds 6
- EnhancedVisuals
- Hit Feedback
- Lucent
- Presence Footsteps (Forge)
- Sound Physics Remastered
- Stylish Effects
- [LEGACY] Drip Sounds (Forge)

### ライブラリ・前提 Mod

- Architectury API
- Atlas Lib
- Balm
- Bookshelf
- Citadel
- Cloth Config API (Fabric/Forge/NeoForge)
- CoroUtil
- CreativeCore
- Cupboard
- Curios API (Forge/NeoForge)
- Forgified Fabric API
- Framework
- GeckoLib
- Iguana Lib
- InsaneLib
- Knight Lib
- Kotlin for Forge
- Majrusz Library
- Moonlight Lib
- Patchouli
- Placebo
- playerAnimator
- PolyLib
- Puzzles Lib
- VLib

---

## 進行ルール案

### 基本ルール

- 地上に恒久拠点を作ってはいけない。
- 地上に置けるのは仮設キャンプ、採掘前哨基地、防衛塔、補給拠点のみ。
- AE2 のメインストレージは空中拠点のみ。
- Mekanism の上位機械は空中拠点のみ。
- Digital Miner は中盤後半まで禁止。
- MekaSuit は終盤まで禁止。
- 長距離移動や補給路は飛行船、JourneyMap、Nature's Compass を前提に計画する。
- 地上での長時間放置は禁止。
- ホード発生時、地上施設は放棄してもよい。

---

## クエストサンプル

FTB Quests などで実装することを想定した、章ごとのクエスト案です。

---

### 第0章：墜落 / Crash Landing

目的: 最低限の生存準備を整え、地上が安全ではないことを理解する。

- 木材を入手する
- 石ツール一式を作る
- 食料を確保する
- 仮設キャンプを作る
- 初日の夜を生き延びる
- 地上に恒久拠点を作らないルールを確認する

報酬案:

- 松明
- パン
- 包帯や簡易回復アイテム
- 初期バックパック

---

### 第1章：地上サバイバル / Ground Survival

目的: 地上で最低限の工業準備を行い、空へ逃げるための素材を集める。

- 鉄を入手する
- 銅を入手する
- Create の Andesite Alloy を作る
- Create の水車または風車を作る
- Immersive Engineering の Engineer's Hammer を作る
- Coke Oven を作る
- Treated Wood を作る
- 地上の廃墟またはダンジョンを1か所探索する

報酬案:

- Create 用素材
- IE 用素材
- 弾薬少量
- 地図またはコンパス

---

### 第2章：初期工業化 / First Industry

目的: 空中拠点建造に必要な基本工業を整える。

- Create の Mechanical Press を作る
- Mechanical Mixer を作る
- Crushing Wheel を作る
- IE の LV Wire Coil を作る
- IE の基本発電設備を作る
- 初期の鉱石処理ラインを作る
- 地上に採掘前哨基地を1つ作る

報酬案:

- 金属素材
- Engineer's Blueprint
- Create の Shaft / Cogwheel セット

---

### 第3章：初飛行 / First Flight

目的: 地上から脱出し、最初の飛行船または空中拠点を作る。

- Valkyrien Skies / Eureka 用の基本パーツを作る
- 小型飛行船を建造する
- 飛行船に作業台・チェスト・ベッドを設置する
- 初飛行に成功する
- 地上の主要アイテムを空へ移す
- 空中拠点の建造を開始する

報酬案:

- 建材
- 予備燃料
- 修理素材
- 地上探索用の弾薬

---

### 第4章：空中拠点化 / Sky Base

目的: 空中で生活・工業・保管ができる状態を作る。

- 空中拠点に農業区画を作る
- 空中拠点に発電区画を作る
- 空中拠点に加工区画を作る
- 空中拠点に倉庫区画を作る
- AE2 の ME Chest を作る
- AE2 の ME Drive を作る
- 空中拠点の常時稼働させたい区画を決める

報酬案:

- AE2 素材
- 建築ブロック
- 食料生産用アイテム

---

### 第5章：武装降下作戦 / Armed Expedition

目的: 地上探索を本格化し、危険地帯から戦利品を持ち帰る。

- TaCZ の銃を入手または作成する
- 弾薬を生産する
- Superb Warfare の基本武器を作る
- 地上の大型構造物を1つ攻略する
- 廃墟都市からレア素材を回収する
- 降下地点と撤退ルートを整備する
- 地上前哨基地を防衛可能な状態にする

報酬案:

- 武器パーツ
- 弾薬
- 希少鉱石
- 探索用バックパック強化

---

### 第6章：自動化と大量生産 / Automation

目的: 空中拠点を本格的な工業拠点へ発展させる。

- AE2 の自動クラフトを開始する
- Mekanism の基本機械を作る
- Mekanism の鉱石2倍化ラインを作る
- Mekanism の鉱石3倍化ラインを作る
- Create の自動加工ラインを作る
- 弾薬生産を自動化する
- 地上探索用の補給セットを自動生産する

報酬案:

- 上位機械素材
- AE2 Processor 素材
- Mekanism Alloy

---

### 第7章：空中要塞 / Sky Fortress

目的: 空中拠点を防衛・生産・探索のすべてに対応した要塞へ拡張する。

- 空中拠点に武器庫を作る
- 空中拠点に弾薬庫を作る
- 空中拠点に Superb Warfare / TaCZ Turrets などの防衛設備を設置する
- 小型降下艇または探索船を作る
- Mekanism の上位鉱石処理ラインを作る
- 空中拠点に大型発電設備を作る
- 複数の地上前哨基地を補給ネットワーク化する

報酬案:

- 高級武器素材
- 防衛設備素材
- 大量建材

---

### 第8章：地上奪還 / Reclaim the Surface

目的: 空中要塞の力で、危険な地上を攻略・制圧する。

- 大型ダンジョンを3つ攻略する
- 廃墟都市を1つ制圧する
- 地上に防衛済み採掘前哨基地を3つ作る
- Mekanism の核分裂炉を稼働させる
- AE2 の主要素材自動クラフトを完成させる
- 最終遠征用の補給物資を準備する
- 地上に記念碑またはビーコンを建てる

報酬案:

- クリア記念アイテム
- 装飾ブロック
- 終盤装備素材

---

## 最終クリア条件

以下をすべて達成したら、この Modpack のメインクリアとする。

- 空中工業要塞 **Sky Fortress** を完成させる
- AE2 メインストレージを空中拠点に構築する
- AE2 自動クラフトを実用化する
- Mekanism の高性能鉱石処理ラインを空中拠点に構築する
- 大型発電設備を稼働させる
- 武器庫と弾薬生産ラインを完成させる
- 地上の大型ダンジョンを3つ攻略する
- 廃墟都市または危険地帯を1つ制圧する
- 地上に防衛済み前哨基地を3つ作る
- 最終遠征後、地上にビーコンまたは記念碑を建てる

---

## クリア後目標

- Mekanism 核融合炉を稼働させる
- MekaSuit を完成させる
- 主要素材を AE2 で完全自動クラフト化する
- 空中戦艦を建造する
- 小型飛行船・降下艇・輸送船を複数建造する
- すべての追加バイオームを探索する
- すべての主要ダンジョン系構造物を攻略する
- 地上に安全地帯を復興する

---

## メモ

この Modpack は、単に工業を進めるだけでなく、以下のループを重視します。

1. 空中拠点で準備する
2. 地上へ降下する
3. 資源・戦利品・設計素材を回収する
4. 危険になる前に撤退する
5. 空中拠点を拡張する
6. より危険な地上へ挑む

工業化は目的ではなく、地上を攻略するための手段です。
